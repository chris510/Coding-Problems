...
//         let includeLeftNum = skipLeft + nums[i];
//         let doNotIncludeNum = table[i - 1];
//         table[i] = Math.max(includeLeftNum, doNotIncludeNum);
//     }
//     return table[table.length - 1];
// }
...
const maxNonAdjacentSum = (nums, memo={}) => {
    ...
    let firstEl = nums[0];
    memo[nums.length] = Math.max(firstEl + maxNonAdjacentSum(nums.slice(2), memo), maxNonAdjacentSum(nums.slice(1), memo));
